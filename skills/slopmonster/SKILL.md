...
> all. `${CLAUDE_PLUGIN_ROOT}/references/short-messages.md` is the receipt and the passes.

For a short message, run `${CLAUDE_PLUGIN_ROOT}/references/short-messages.md` FIRST: reply shape, hard cuts,
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/tools/deslop.py page.html              # a built page (scores visible text only)
python3 ${CLAUDE_PLUGIN_ROOT}/tools/deslop.py page.html --view hero  # one element by id
python3 ${CLAUDE_PLUGIN_ROOT}/tools/deslop.py --text "paste a draft"
...
Touching a regex means running `python3 ${CLAUDE_PLUGIN_ROOT}/tools/test_deslop.py`. The catalogue is matched by
word root, and the obvious stemming shortcut silently kills a dozen base words.

## Step 2 — Rewrite (three passes)

Full catalogue in `${CLAUDE_PLUGIN_ROOT}/references/signs-of-ai-writing.md`. The short version:
...
|---|---|---|
| Claude Code / Claude | Anthropic | GPT-5.6 via the codex CLI — `${CLAUDE_PLUGIN_ROOT}/tools/cleanse.sh` does this |
...
```bash
${CLAUDE_PLUGIN_ROOT}/tools/cleanse.sh draft.md > cleansed.md                 # copy out, notes on stderr
${CLAUDE_PLUGIN_ROOT}/tools/cleanse.sh draft.md > cleansed.md 2> notes.txt    # keep the notes as well
```

The instruction it carries (`${CLAUDE_PLUGIN_ROOT}/prompts/cleanse.txt`): strip the tells, keep every fact,
